...
export class GitOperations {
     ...
     */
    async pushBranchForceWithLease(branchName: string, remoteName: string = "origin", setUpstream: boolean = false): Promise<void> {
     ...
     * 使用Git原生命令验证分支名合法性
     */
    async validateBranchNameWithGit(branchName: string): Promise<{
        isValid: boolean;
        error?: string;
    }> {
     ...
     */
    async getCherryPickCandidates(sourceBranch: string, maxCount: number = 50): Promise<CherryPickCommitInfo[]> {
     ...
     */
    async cherryPickCommit(commitHash: string, recordOrigin: boolean = true): Promise<CherryPickOutcome> {
     ...
     */
    async getLatestIncomingMergeCommit(branchName?: string): Promise<LatestMergeCommit | null> {
        ...
        const mergedBranchHint = await this.resolveBranchHintForCommit(parentHashes[1], branch);
        return {
            hash,
            subject,
            parentHashes,
     ...
     * 根据提交解析可读的引用名（优先本地分支）
     */
    private async resolveBranchHintForCommit(commitHash: string, excludeBranch?: string): Promise<string | undefined> {
        try {
     ...
     */
    async previewRevertMergeConflicts(commitHash: string, mainlineParent: number = 1): Promise<string[]> {
        try {
            await this.execGitArgs([
                "revert",
                "-m",
     ...
     */
    async revertMergeCommitSafe(commitHash: string, mainlineParent: number = 1): Promise<RevertMergeResult> {
        try {
            await this.execGitArgs([
                "revert",
                "-m",
     ...
     */
    async setBranchCreationBase(branchName: string, baseBranch: string): Promise<void> {
        await this.execGitArgs([
            "config",
     ...
     * 解析当前分支基于哪个分支创建
     */
    async resolveBranchBaseInfo(branchName: string, extraCandidates: string[] = []): Promise<BranchBaseInfo> {
    ...
    private async inferBaseFromReflog(branchName: string): Promise<{
        ...
        for (const line of chronological) {
            ...
            if (createdMatch) {
                ...
                const pointingAt = await this.getBranchesPointingAtCommit(hash, branchName);
    ...
    private async getBranchesPointingAtCommit(commitHash: string, excludeBranch: string): Promise<string[]> {
        try {
            const branches = await this.execGitArgs([
                "branch",
                "-a",
    ...
    private async inferBaseFromForkPoint(branchName: string, candidates: string[]): Promise<{
        baseBranch?: string;
        baseCommit?: string;
        candidates?: string[];
    }> {
        ...
        for (const candidate of candidates) {
            ...
            try {
                ...
                const behind = parseInt(await this.execGitArgs([
                    "rev-list",
                    "--count",
                    `${mergeBase}..${candidate}`,
                ]), 10);
                const ahead = parseInt(await this.execGitArgs([
                    "rev-list",
                    "--count",
        ...
        const best = scores[0];
        const ties = scores.filter((s) => s.behind === best.behind && Math.abs(s.ahead - best.ahead) <= 2);
        if (ties.length > 1) {
