...
export class GitMergeService {
    ...
    private getFeatureBranchConfig(): FeatureBranchConfig {
        ...
        return config.get<FeatureBranchConfig>('featureBranchConfig', defaultConfig);
    }
    ...
    private async getMainBranch(): Promise<string> {
        ...
        if (autoDetect) {
            try {
                ...
                vscode.window.showWarningMessage(
                    '未找到标准的主分支(main/master/release/develop)，请手动配置主分支'
                );
            } catch (error) {
        ...
        // 使用配置的主分支
        const configuredBranch = config.get<string>('mainBranch', 'master');
     ...
     */
    private async handleMergeConflicts(): Promise<boolean> {
     ...
     */
    private async safeMergeBranch(targetBranch: string, sourceBranch: string): Promise<boolean> {
        try {
     ...
     * @returns Promise<void>
     */
    private async configureFeatureBranchPatterns(): Promise<void> {
        const config = this.getConfiguration();
        ...
        switch (action.label) {
            ...
            case '删除模式':
                ...
                const patternToRemove = await vscode.window.showQuickPick(
                    currentConfig.patterns.map(pattern => ({ label: pattern })),
                    { placeHolder: '选择要删除的分支模式' }
                );
        ...
        if (shouldValidateRemote?.label === '是') {
            try {
                ...
                const shouldContinue = await vscode.window.showWarningMessage(
                    `分支 "${branchName}" 在远程仓库中不存在，是否仍要添加？`,
                    '继续添加',
                    '取消'
                );
    ...
    private async removeTargetBranch(): Promise<void> {
        ...
        const confirm = await vscode.window.showWarningMessage(
            `确定要删除目标分支 "${branchToRemove.label}" 吗？`,
            '确定删除',
            '取消'
        );
    ...
    private async resetConfiguration(): Promise<void> {
        ...
        const confirm = await vscode.window.showWarningMessage(
            `确定要${resetOptions.label}吗？此操作不可撤销。`,
            '确定重置',
            '取消'
        );
        ...
        try {
            ...
            // 显示重置后的配置信息
            const shouldShowConfig = await vscode.window.showInformationMessage(
                '配置重置完成，是否查看当前配置？',
                '查看配置',
                '关闭'
            );
     ...
     */
    public async validateGitEnvironment(): Promise<{isValid: boolean, issues: string[]}> {
        const issues: string[] = [];
