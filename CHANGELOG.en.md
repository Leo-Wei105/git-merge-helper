...
- **Default Language Switched to Chinese**: The extension's default language is now Chinese, including command names, configuration descriptions, and runtime prompts. English users automatically load English language packs via VS Code language settings.
- **Documentation Defaults to Chinese**: `README.md` and `CHANGELOG.md` now display Chinese content by default. English versions moved to `README.en.md` and `CHANGELOG.en.md`.
- **File Cleanup**: Removed redundant files (`package.nls.zh-cn.json`, `l10n/bundle.l10n.zh-cn.json`, `README.zh-CN.md`, `CHANGELOG.zh-CN.md`) to streamline the project structure.
